...
//global accumalted error out
unsigned short errorout[1024];

//global uart output device
File *UartDevice_ptr;
...
int main(int argc, char *argv[]){
	set_up_uart();
	set_up_dma(DMA_DEV_ID); //Jeff's edits --> sets up dma transfer sys
...
int process_im(int *img, int *th_img, int *th_pos, int alpha, int beta){
	...
	*/
	...
	//>>JEFF'S ADDITION --> print log modified, adds in new marker and param to errorout
	print_log(); //--> needs to be changed to UART instead of stdout
	send_acs();
